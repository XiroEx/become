...
/** The fix, mirrored: per-unit values keep four decimals. */
const PER_UNIT_DP = 10_000
const roundPerUnit = (v: number | undefined) => Math.round((v ?? 0) * PER_UNIT_DP) / PER_UNIT_DP
...
test('the calories now survive the round trip', () => {
  assert.equal(roundPerUnit(perUnitCal) * UNITS, TOTAL_CAL)
  assert.equal(roundPerUnit(perUnitProtein) * UNITS, TOTAL_PROTEIN)
})
...
test('it holds for anything measured in many small units', () => {
  ...
  for (const [total, units] of [[150, 250], [45, 1000], [8, 500], [600, 340]] as const) {
    ...
    assert.equal(Math.round(roundPerUnit(per) * units * 100) / 100, total, `${total} over ${units} units`)
  }
})
...
test('a whole-unit food was never affected, which is why this hid', () => {
  ...
  assert.equal(roundPerUnit(140) * 2, 280)
})

test('every plate-scan write path uses the precise helper', () => {
  ...
  assert.doesNotMatch(src, /Math\.round\(n\.calories/, 'a per-unit calorie is still being rounded to an integer')
  assert.equal((src.match(/calories: roundPerUnit\(n\.calories\)/g) ?? []).length, 3)
})
