...
// CoachChat — the shared AI chat surface for every freeform consultant (mindset /
// nutrition / training). It is DURABLE: the reply is generated by the app-level
// run store, and the conversation + the in-flight runId are persisted to
// localStorage under `persistKey`. So you can send a message, close the modal or
// leave the app, come back, and the reply is either still cooking (the global
// pill shows it) or already here waiting — nothing is lost when the component
// unmounts.
//
// It POSTs to `endpoint` with { domain?, message, history, grounding, conversationId }
// and the run resolves to { text }. One component, three surfaces.
...
export default function CoachChat({
  ...
  suggestions?: string[]
  /** Stable key → conversation + in-flight run survive close/reopen + reload. */
  persistKey?: string
  /** Label shown in the global activity pill while a reply generates. */
  runLabel?: string
  onClose: () => void
}) {
  const storageKey = persistKey ? `become.chat.${persistKey}` : null

  const restore = (): PersistedChat => {
    if (storageKey && typeof window !== 'undefined') {
      try {
        const raw = localStorage.getItem(storageKey)
        if (raw) {
          const d = JSON.parse(raw) as PersistedChat
          if (Array.isArray(d.messages) && d.messages.length) {
            return { messages: d.messages, pendingRunId: typeof d.pendingRunId === 'string' ? d.pendingRunId : null }
          }
        }
      } catch { /* ignore */ }
    }
    return { messages: [{ role: 'assistant', text: greeting }], pendingRunId: null }
  }

  const initial = useRef(restore())
  const [messages, setMessages] = useState<ChatMessage[]>(initial.current.messages)
  const [pendingRunId, setPendingRunId] = useState<string | null>(initial.current.pendingRunId)
  ...
  // Observe the in-flight run (reattaches by id on reopen/reload).
  const run = useAiRun(pendingRunId)
  const sending = !!pendingRunId && (!!run && run.status === 'pending')

  // Persist conversation + pending run whenever they change.
  useEffect(() => {
    if (!storageKey) return
    try {
      localStorage.setItem(storageKey, JSON.stringify({ messages, pendingRunId } satisfies PersistedChat))
    } catch { /* quota — non-fatal */ }
  }, [messages, pendingRunId, storageKey])

  // When the pending run finishes (now, or after we reopened), append the reply.
  useEffect(() => {
    if (!pendingRunId) return
    if (!run) { setPendingRunId(null); return } // run no longer tracked (expired)
    if (run.status === 'pending') return
    const reply = cleanReply((run.text && run.text.trim()) || '')
  ...
  const send = async (text: string) => {
    ...
    const id = await runStore.start(
      endpoint,
      { domain, message: msg, history, grounding, conversationId: convoId.current },
      { kind: domain ? `consultant.${domain}` : 'coach', label: runLabel, meta: { persistKey } },
    )
    if (id) setPendingRunId(id)
    else setMessages((m) => [...m, { role: 'assistant', text: 'Connection hiccup — give that another try in a sec.' }])
  }
