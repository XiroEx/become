    ...
    <PageTransition className="space-y-4 pb-24">
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="p-4">
          ...
          {meal.description && (
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{meal.description}</p>
          )}
      ...
      {meal.recipe && meal.recipe.instructions?.length > 0 && (
        <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <h2 className="mb-2 text-sm font-semibold text-zinc-700 dark:text-zinc-300">Recipe</h2>
          <div className="mb-3 flex flex-wrap gap-3 text-xs text-zinc-600 dark:text-zinc-400">
            {meal.recipe.prepTimeMinutes != null && (
              <span className="inline-flex items-center gap-1">
                <Clock className="h-3 w-3" /> Prep {meal.recipe.prepTimeMinutes}m
              </span>
            )}
            {meal.recipe.cookTimeMinutes != null && (
              <span className="inline-flex items-center gap-1">
                <Clock className="h-3 w-3" /> Cook {meal.recipe.cookTimeMinutes}m
              </span>
            )}
            {meal.recipe.servings != null && (
              <span className="inline-flex items-center gap-1">
                <Users className="h-3 w-3" /> Yields {meal.recipe.servings}
              </span>
            )}
          </div>
