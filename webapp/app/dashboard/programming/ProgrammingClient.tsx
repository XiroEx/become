  ...
  return (
    <>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-zinc-600">{program.goal} • {program.duration_weeks} weeks</p>
                </div>
                <svg 
      ...
      {selectedProgram && (
        ...
        >
          <div 
            className="w-full max-w-4xl rounded-2xl bg-white max-h-[90vh] overflow-hidden flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="p-6 border-b border-zinc-100 flex items-start justify-between bg-white sticky top-0 z-10">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-zinc-900">{selectedProgram.name}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="mb-8">
                ...
                {currentPhase && (
                    <p className="mt-3 text-sm text-zinc-600 bg-zinc-50 p-3 rounded-lg border border-zinc-100">
                        <span className="font-semibold text-zinc-900">Focus:</span> {currentPhase.focus}
                    </p>
                )}
              </div>
              ...
              {currentWorkout ? (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                  <div>
                    <h3 className="text-xl font-bold text-zinc-900">{currentWorkout.title}</h3>
                  </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-1">
                    {currentWorkout.exercises.map((exercise, index) => (
                      <div 
                        key={index}
                        className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4 transition-colors hover:border-zinc-300"
                      >
                        <div className="flex items-start gap-4">
                          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white border border-zinc-200 text-sm font-bold text-zinc-900 shadow-sm">
                            {index + 1}
                          </span>
                          <div>
                            <h4 className="font-semibold text-zinc-900">{exercise.name}</h4>
                            {exercise.type === 'conditioning' ? (
                                <p className="text-sm text-zinc-600 mt-1">{exercise.details}</p>
                            ) : (
                                <div className="flex gap-4 mt-1 text-sm text-zinc-600">
                        ...
                        {exercise.rest && (
                            <div className="flex items-center gap-2 text-xs font-medium text-zinc-500 bg-white px-3 py-1.5 rounded-full border border-zinc-100 shadow-sm self-start sm:self-center">
            ...
            <div className="p-6 border-t border-zinc-100 bg-zinc-50">
                <button
                onClick={() => setSelectedProgram(null)}
                className="w-full cursor-pointer rounded-xl bg-zinc-900 py-3 font-semibold text-white transition-colors hover:bg-black shadow-lg shadow-zinc-900/10"
                >
                Close Program
                </button>
            </div>
