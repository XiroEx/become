...
// ─── 50-level layer (5 tiers × 10) ───────────────────────────────────────────
// The 5 CHAPTERS above are the TIERS. Each tier now spans 10 levels, so the arc
// reads as 50 granular levels — more frequent, motivating progress — while the
// segment-unlock GATING stays per-tier (= per chapter, unchanged). Level is
// derived from total XP; tier = ceil(level / 10). Tier boundaries line up with
// the old chapter XP thresholds, so what unlocks when doesn't change.

export const LEVELS_PER_TIER = 10
export const MAX_LEVEL = 50

// XP to ENTER each tier (index = tier - 1) + a top sentinel for tier-5's inner
// levels. First five match CHAPTER_XP_THRESHOLDS so gating is identical.
const TIER_START_XP = [0, 50, 150, 300, 500, 1000]

// Cumulative XP to reach each level 1..50 (index = level - 1). Within a tier the
// ten levels are spaced evenly between that tier's start XP and the next tier's.
export const LEVEL_XP: number[] = (() => {
  const out: number[] = []
  for (let tier = 1; tier <= 5; tier++) {
    const start = TIER_START_XP[tier - 1]
    const per = (TIER_START_XP[tier] - start) / LEVELS_PER_TIER
    for (let i = 0; i < LEVELS_PER_TIER; i++) out.push(Math.round(start + i * per))
  }
  return out
})()

export function tierForLevel(level: number): number {
  return Math.min(5, Math.max(1, Math.ceil(level / LEVELS_PER_TIER)))
}
...
/** Full level/tier progress for the hub — derived purely from total XP. */
export function getLevelProgress(xp: number): LevelProgress {
  const level = levelForXp(xp)
  const tier = tierForLevel(level)
  const atMax = level >= MAX_LEVEL
  const curThresh = LEVEL_XP[level - 1]
  const nextThresh = atMax ? curThresh : LEVEL_XP[level]
  const xpForLevel = Math.max(1, nextThresh - curThresh)
  const xpIntoLevel = Math.max(0, xp - curThresh)
  return {
    level,
    tier,
    tierName: CHAPTERS[tier - 1].name,
    xpIntoLevel,
    xpForLevel,
    pct: atMax ? 100 : Math.min(100, Math.round((xpIntoLevel / xpForLevel) * 100)),
    xpToNext: atMax ? 0 : Math.max(0, nextThresh - xp),
    atMax,
  }
}
